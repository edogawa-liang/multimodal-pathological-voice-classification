...
def train_model(model_name, X_train, y_train, X_test, y_test):
    ...
    if model_name == "SVM":
        ...
        model = XGBClassifier(objective='multi:softmax', num_class=len(set(y_train)), random_state=42, class_weight='balanced')
    elif model_name == "LGBM":
        model = LGBMClassifier(objective='multiclass', num_class=len(set(y_train)), random_state=42, is_unbalance=True)
    elif model_name == "Logistic":
        model = LogisticRegression(solver='lbfgs', multi_class='multinomial', random_state=42, class_weight='balanced')
    else:
